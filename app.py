...
def generate_plots(N=100, mu=1, sigma2=4, S=100):
    ...
    plt.title("Linear Regression of X vs Y under mu = " + str(mu) + " sigma2 = "+str(sigma2))
    plt.xlabel("X")
    plt.ylabel("Y")
    plt.legend()
    plt.grid(True)
    ...
    for _ in range(S):
        ...
        X_sim =np.random.rand(N).reshape(-1,1)  # Replace with code to generate X values
        ...
        slope1 = sim_model.coef_[0]  # Replace with code to extract slope from the fitted model
        intercept1 = sim_model.intercept_  # Replace with code to extract intercept from the fitted model

        # TODO: Append the slope and intercept of the model to slopes and intercepts lists
        slopes.append(slope1)  # Replace None with code to append slope
        intercepts.append(intercept1)  # Replace None with code to append intercept
